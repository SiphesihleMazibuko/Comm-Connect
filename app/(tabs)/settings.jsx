...
export default function SettingsScreen() {
  ...
  const performLogout = async () => {
    ...
    try {
      ...
      if (sessionError) console.warn('[LOGOUT] Could not read current session:', sessionError);
  ...
  return (
    <View style={{ flex: 1, backgroundColor: colors.background }}>
      <ScrollView style={{ flex: 1, backgroundColor: colors.background }} contentContainerStyle={{ paddingBottom: 40 }} showsVerticalScrollIndicator={false}>
        <ScreenHeader title={fullName || 'Settings'} subtitle={userData?.email || 'Manage your account'} meta={`Role: ${roleLabel}`} icon="person" />
        <View style={{ paddingHorizontal: 16, paddingTop: 8 }}>
          <Text style={{ fontSize: 13, fontWeight: '800', color: colors.textLight, marginTop: 8, marginBottom: 10, letterSpacing: 0.7 }}>ACCOUNT</Text>
          ...
          <Text style={{ fontSize: 13, fontWeight: '800', color: colors.textLight, marginBottom: 10, letterSpacing: 0.7 }}>PREFERENCES</Text>
          <View style={{ backgroundColor: colors.surface, borderRadius: 18, borderWidth: 1, borderColor: colors.border, paddingHorizontal: 16, marginBottom: 24 }}>
            <SettingItem icon={isDark ? 'moon' : 'sunny'} title="Dark Mode" subtitle={isDark ? 'Dark theme enabled' : 'Light theme enabled'}>
              <Switch value={isDark} onValueChange={toggleTheme} trackColor={{ false: colors.border, true: colors.primary }} thumbColor="#FFFFFF" ios_backgroundColor={colors.border} />
            </SettingItem>
            <SettingItem icon="eye-off-outline" title="Anonymous Reporting" subtitle="Default to anonymous when reporting incidents">
              <Switch value={anonymousDefault} onValueChange={toggleAnonymousDefault} trackColor={{ false: colors.border, true: colors.primary }} thumbColor="#FFFFFF" ios_backgroundColor={colors.border} />
            </SettingItem>
          </View>

          <Text style={{ fontSize: 13, fontWeight: '800', color: colors.textLight, marginBottom: 10, letterSpacing: 0.7 }}>SOS CONTACTS</Text>
          <View style={{ backgroundColor: colors.surface, borderRadius: 18, borderWidth: 1, borderColor: colors.border, padding: 16, marginBottom: 24 }}>
            <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 14 }}>
              ...
              <View style={{ flex: 1 }}>
                ...
                <Text style={{ fontSize: 12, color: colors.textLight, marginTop: 3, lineHeight: 17 }}>These contacts can receive your SOS emergency message.</Text>
              </View>
            </View>
            ...
            {emergencyContacts.length === 0 ? (
              <View style={{ backgroundColor: colors.surfaceSoft || colors.primaryLight, borderRadius: 14, padding: 18, alignItems: 'center', marginBottom: 12, borderWidth: 1, borderColor: colors.border }}>
                ...
                <Text style={{ color: colors.text, fontSize: 14, fontWeight: '700', marginTop: 8, textAlign: 'center' }}>No emergency contacts</Text>
                <Text style={{ color: colors.textLight, fontSize: 12, marginTop: 5, textAlign: 'center', lineHeight: 18 }}>Add at least one emergency contact before using SOS.</Text>
              </View>
            ) : (
              emergencyContacts.map((contact) => (
                <View key={contact.id} style={{ backgroundColor: colors.surfaceSoft || colors.background, borderRadius: 14, padding: 13, marginBottom: 9, borderWidth: 1, borderColor: colors.border }}>
                  <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                    ...
                    <TouchableOpacity onPress={() => removeEmergencyContact(contact.id)} style={{ width: 38, height: 38, borderRadius: 12, alignItems: 'center', justifyContent: 'center', backgroundColor: colors.errorLight || `${colors.error}15` }}>
            ...
            <TouchableOpacity onPress={() => { setContactDraft(EMPTY_CONTACT); setShowContactModal(true); }} style={{ backgroundColor: colors.primary, borderRadius: 13, paddingVertical: 14, alignItems: 'center', justifyContent: 'center', flexDirection: 'row', gap: 8, marginTop: 3 }}>
              <Ionicons name="person-add" size={18} color="#FFFFFF" />
              <Text style={{ color: '#FFFFFF', fontSize: 14, fontWeight: '800' }}>Add Emergency Contact</Text>
            </TouchableOpacity>
          </View>

          <TouchableOpacity onPress={handleLogout} disabled={loggingOut} style={{ backgroundColor: colors.error, borderRadius: 14, paddingVertical: 15, alignItems: 'center', justifyContent: 'center', flexDirection: 'row', gap: 9, marginBottom: 10, opacity: loggingOut ? 0.7 : 1 }}>
            {loggingOut ? <ActivityIndicator color="#FFFFFF" /> : (
              <>
        ...
        <Modal visible={showContactModal} transparent animationType="fade" onRequestClose={() => { setContactDraft(EMPTY_CONTACT); setShowContactModal(false); }}>
          <View style={{ flex: 1, backgroundColor: 'rgba(0,0,0,0.6)', justifyContent: 'center', alignItems: 'center', padding: 16 }}>
            <View style={{ width: '100%', maxWidth: 500, backgroundColor: colors.surface, borderRadius: 22, padding: 20, borderWidth: 1, borderColor: colors.border }}>
              <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 18 }}>
                ...
                <TouchableOpacity onPress={() => { setContactDraft(EMPTY_CONTACT); setShowContactModal(false); }} style={{ width: 36, height: 36, borderRadius: 12, backgroundColor: colors.primaryLight, alignItems: 'center', justifyContent: 'center' }}>
                  <Ionicons name="close" size={20} color={colors.primary} />
                </TouchableOpacity>
              </View>

              <Text style={{ color: colors.text, fontSize: 12, fontWeight: '700', marginBottom: 6 }}>NAME</Text>
              <TextInput style={{ backgroundColor: colors.surfaceLight || colors.background, color: colors.text, borderColor: colors.border, borderWidth: 1, borderRadius: 12, paddingHorizontal: 14, paddingVertical: 13, marginBottom: 12, fontSize: 14 }} placeholder="e.g. Jane Smith" placeholderTextColor={colors.inputPlaceholder || colors.textLight} selectionColor={colors.primary} value={contactDraft.name} onChangeText={(value) => setContactDraft((previous) => ({ ...previous, name: value }))} autoCapitalize="words" />
              ...
              <View style={{ flexDirection: 'row', gap: 10 }}>
                ...
                <TouchableOpacity onPress={addEmergencyContact} disabled={savingContact} style={{ flex: 1, backgroundColor: colors.primary, borderRadius: 12, paddingVertical: 13, alignItems: 'center', justifyContent: 'center', opacity: savingContact ? 0.7 : 1 }}>
                  {savingContact ? <ActivityIndicator color="#FFFFFF" /> : <Text style={{ color: '#FFFFFF', fontWeight: '800' }}>Save Contact</Text>}
                </TouchableOpacity>
              </View>
            </View>
          </View>
