...
const StatsGauge = ({ title, value, subtitle, icon, colors, isDark, style }) => {
  const maxReports = 50;
  ...
  return (
    <View style={[{ width: '48.5%', marginBottom: 14, borderRadius: 22, overflow: 'hidden', backgroundColor: colors.surface, borderWidth: 1, borderColor: colors.border }, style]}>
...
export default function PendingReportsScreen() {
  ...
  async function fetchPendingReports(profile = userProfile) {
    try {
      let reportsData;
      let wardReportsData;
          ...
          <View style={{ paddingHorizontal: 16, marginTop: 18 }}>
            ...
            <View style={{ flexDirection: 'row', flexWrap: 'wrap', justifyContent: 'space-between' }}>
              {crimeMatrix.map((category) => (
                <TouchableOpacity key={category.id} onPress={() => openCrimeCategory(category)} activeOpacity={0.86} style={{ width: '48.5%' }}>
                  <StatsGauge
