...
export default function SignupHeader({ step }) {
  ...
  return (
    <View style={{ alignItems: 'center', marginBottom: 32 }}>
      ...
      <Text style={{ fontSize: 28, fontWeight: 'bold', color: colors.text }}>{step === 'details' ? 'Create Account' : 'Verify Phone'}</Text>
      <Text style={{ fontSize: 14, color: colors.textLight, marginTop: 8 }}>{step === 'details' ? 'Join Comm-Connect today' : 'Enter the code sent to your phone'}</Text>
    </View>
  );
}